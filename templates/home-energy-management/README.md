...
Parameterized template set for the **`home-energy-management`** blueprint
...
A personalized, community-driven home energy management web app with:

- Hero + sticky **Header** with theme toggle and anchor navigation
- **Energy dashboard** — real-time consumption, peak periods, and trend bars
- **Savings recommendations** — AI-personalized ways to cut usage and cost
- **Utility bill analyzer** — upload bills, spot anomalies, see savings insights
- **Device automation** — toggle smart thermostats, plugs, and appliances
- **Home audit** — interactive virtual energy audit with tailored tips
- **Resource library** — searchable articles, videos, and guides

## Structure

```
templates/home-energy-management/
├── README.md
...
│   │   └── energy/
│   │       ├── DeviceAutomation.tsx.template
│   │       ├── EnergyDashboard.tsx.template
│   │       ├── HomeAudit.tsx.template
│   │       ├── ResourceLibrary.tsx.template
│   │       ├── SavingsRecommendations.tsx.template
│   │       └── UtilityBillAnalyzer.tsx.template
│   └── data/home-energy-management.json.template  ← realistic sample content
```
...
| `frontend/backend/ui/state/auth/security/ai/devices/email/analytics/api/calendar/gamification` | framework & provider choices | prompt.md feature lines |

Business content is **never hard-coded** in components: consumption series,
savings tips, bills, devices, audit questions and resources flow through
`src/data/home-energy-management.json`, which `src/app/page.tsx` imports and
...
4. Emit a runnable project; `{{app.slug}}` in the page/data/package.json
   imports and the import path `@/data/home-energy-management.json` align by
   construction.
