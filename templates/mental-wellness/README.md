# Mental Wellness Platform — Template Set

A parameterized, buildable skeleton for a **Personalized Mental Wellness and Self-Care Platform** (`slug: mental-wellness`, archetype `PersonalizedVertical`). The instance generator reads the blueprint record (`blueprint.json.template`) and resolves every Mustache token across the `.template` files to emit a ready-to-build Next.js project.
...
| `src/app/page.tsx.template` | Main entry — assembles all sections from `src/data/mental-wellness.json`. |
| `src/data/mental-wellness.json.template` | Content/config data the page and components consume (mood tracks, wellness plans, resources, community). |
| `src/components/layout/Header.tsx.template` | Sticky, responsive header with theme toggle and mobile menu. |
| `src/components/wellness/MoodTracker.tsx.template` | Mood tracking dashboard with daily entries and visual patterns. |
| `src/components/wellness/WellnessPlan.tsx.template` | Personalized wellness plan generator with selected activities. |
| `src/components/wellness/ResourceLibrary.tsx.template` | Resource library with articles, videos, and guided meditations. |
| `src/components/wellness/SupportForum.tsx.template` | Support community forum for sharing and peer support. |
...
- Business content (mood tracks, wellness plans, resources) lives in `src/data/mental-wellness.json.template`, so users swap sample data without touching components.

## Generating the app

```bash
generator load templates/mental-wellness/blueprint.json.template
generator generate mental-wellness --out ./out/mental-wellness
```
...
- Add wellness plans or resources by extending the data file — `WellnessPlan` and `ResourceLibrary` render them automatically.
